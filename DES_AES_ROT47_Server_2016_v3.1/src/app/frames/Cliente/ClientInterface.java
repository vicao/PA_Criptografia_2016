...
public class ClientInterface extends javax.swing.JFrame {
    ...
    private void connected(ChatMessage message) {
        ...
        //sys.EscreveLogConsole("ClienteInterface - Metodo Connected", "[Teste][Chave DES] : " + Base64.getEncoder().encodeToString(chaveDES.getEncoded()));
...
*/
    ...
    private void ExibeMsg(int TipoCriptografia) {
        ...
        if (TipoCriptografia == 0) {
            ...
            //DES ECB
            //resultado = classeDES.DESPaddingECBDecrypter(message.getBytetxt(), chaveDES);//Descriptografa a mensagem
            try {
                //DES CBC
                resultado = classeDES.decryptCBC(message.getBytetxt(), chaveDES.toString());
            } catch (Exception ex) {
                System.out.println(Data + "[Exception]descriptar : " + ex.getMessage());
                sys.EscreveLogConsole("ClienteInterface - Metodo ExibeMsg","[Exception][DES]descriptar : " + ex.getMessage());
            }
        ...
        */
        if (txtAreaSend.getText().isEmpty() || txtAreaSend.getText().equals(null)) {
            ...
            */
            ...
            //sys.EscreveLogConsole("ClienteInterface - Metodo Connected", "[Chave DES] : " + Base64.getEncoder().encodeToString(this.message.getChaveDES().getEncoded()));

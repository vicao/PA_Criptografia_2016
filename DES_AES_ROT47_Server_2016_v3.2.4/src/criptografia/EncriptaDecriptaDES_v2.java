 ...
 public class EncriptaDecriptaDES_v2 {
    ...
    static String Data = new SimpleDateFormat("[dd/MM/yyyy - HH:mm:ss]").format(new Date(System.currentTimeMillis()));
     ...
     public static void main(String[] args) throws Exception {
         ...
         cifraDESECB = Cipher.getInstance("DES/CBC/PKCS5Padding");
    ...
    public static byte[] encryptCBC(String toEncrypt, String key) throws Exception {
        ...
        try {
            ...
            // initialize the cipher with the key and IV
            cifraDESECB.init(Cipher.ENCRYPT_MODE, sk, iv);
            // enctypt!
            encrypted = cifraDESECB.doFinal(toEncrypt.getBytes());
            //****************************************           
            System.out.println("[encryptCBC]iv : " + iv.getIV());

        } catch (Exception ex) {
            System.out.println(Data + "[Exception][encryptCBC] -" + ex.getMessage());
        }
   ...
   public static String decryptCBC(byte[] toDecrypt, String key) throws Exception {
       ...
       try {
           ...
           cifraDESECB.init(Cipher.DECRYPT_MODE, sk, iv);
           decrypted = cifraDESECB.doFinal(toDecrypt);
       } catch (Exception ex) {
           System.out.println(Data + "[Exception][decryptCBC]descriptar : " + ex.getMessage());
       }

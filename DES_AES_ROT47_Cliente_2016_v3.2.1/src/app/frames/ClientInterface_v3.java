...
public class ClientInterface_v3 extends javax.swing.JFrame {
    ...
    static String chaveDESCBC = "password";
    static String chaveAES = null;
    File autolog = new File("C:\\LogsCripto\\");
    String DataArqLog = new SimpleDateFormat("[dd_MM_yyyy-HH_mm_ss]").format(new Date(System.currentTimeMillis()));
    
    String validaCrypt = "";
    byte[] txtbyte = null;
    
    
    public ClientInterface_v3() {
        initComponents();
...
/*--->>>>>>>*/chaveDESCBC = this.message.getChaveDESCBC().toString();
    ...
    private byte[] criptografarDES(int i, String txt) {
        ...
        sys.EscreveLogConsole("ClienteInterface - Metodo criptografar", "******************");
        sys.EscreveLogConsole("ClienteInterface - Metodo criptografar", " Texto : " + txt);

        if (i == 1) {//DES - encryptCBC
            sys.EscreveLogConsole("ClienteInterface - Metodo criptografar[DES]", "[" + this.message.getName() + "]" + "ChavesDES-CBC : " + chaveDESCBC);
            try {
                aux = classeDES.encryptCBC(txt, chaveDES.toString());
                //DES
                sys.EscreveLogConsole("ClienteInterface - Metodo criptografar[DES]", "[" + this.message.getName() + "]" + "Texto Claro : " + txt);
            } catch (Exception ex) {
                Logger.getLogger(ClientInterface_v3.class.getName()).log(Level.SEVERE, null, ex);
            }
        } else if (i == 2) {//DES - DESPaddingECB
            sys.EscreveLogConsole("ClienteInterface - Metodo criptografar[DES]", "[" + this.message.getName() + "]" + "ChavesDES : " + Base64.getEncoder().encodeToString(chaveDES.getEncoded()));
            //Momento que ocorre a encriptação do texto com base na chave DES gerada.            
/*--->>>>>>>*/aux = classeDES.CriptoDESPaddingECB(txt, chaveDES);
        } else if (i == 3) {//AES - CBC
    ...
    private void ExibeMsg(int TipoCriptografia) {
        ...
        sys.EscreveLogConsole("ClienteInterface - Metodo ExibeMsg","***************");
        
        if (TipoCriptografia == 0) {//Sem criptografia
        ...
        } else if (TipoCriptografia == 1) {//DES - encryptCBC

            try {
                //DES CBC
                String resultado = "";
                resultado = classeDES.decryptCBC(message.getBytetxt(), chaveDESCBC);
                
                this.txtAreaReceive.append(message.getName() + " diz : " + resultado + "\n");
                sys.EscreveLogConsole("ClienteInterface - Metodo ExibeMsg[DES CBC]", "Resultado : " + resultado);
                sys.EscreveLogConsole("ClienteInterface - Metodo ExibeMsg[DES CBC]", "Resultado[toString] : " + resultado.toString());
            } catch (Exception ex) {
                System.out.println(DataArqLog + "[ClienteInterface - Metodo ExibeMsg][Exception][DES CBC]descriptar : " + ex.getMessage());
                sys.EscreveLogConsole("ClienteInterface - Metodo ExibeMsg", "[Exception][DES CBC]descriptar : " + ex.getMessage());
            } 

        } else if (TipoCriptografia == 2) {//DES - DESPaddingECB

            //DES ECB
            String resultado = "";            
            resultado = classeDES.CriptoDESPaddingECBDecrypter(message.getBytetxt(), chaveDES);//Descriptografa a mensagem

            this.txtAreaReceive.append(message.getName() + " diz : " + resultado + "\n");
            sys.EscreveLogConsole("ClienteInterface - Metodo ExibeMsg[DES ECB]", "Resultado : " + resultado);
            sys.EscreveLogConsole("ClienteInterface - Metodo ExibeMsg[DES ECB]", "Resultado[toString] : " + resultado.toString());
            //sys.EscreveLogConsole("ClienteInterface - Metodo ExibeMsg[DES]", "Teste[DES] : " +classeDES.descriptar(message.getBytetxt()));
            
        } else if (TipoCriptografia == 3) {//AES - CBC


        } else if (TipoCriptografia == 4) {//AES - ECB

        } else if (TipoCriptografia == 5) {//RSA
    ...
    private void btnEnviarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEnviarActionPerformed
        ...
        sys.EscreveLogConsole("ClienteInterface - Metodo btnEnviar[TpCrypt]","|*******************************************|");
        /*
        ...
        */
        if (txtAreaSend.getText().isEmpty() || txtAreaSend.getText().equals(null)) {
            ...
            */
            ValidaCriptoEnviaMsg(textOld);   

            this.txtAreaReceive.append("Voce disse : " + textOld + "\n");
            sys.EscreveLogConsole("[btnEnviarActionPerformed]","|*********Log Criptografia*******************|");
            sys.EscreveLogConsole("[btnEnviarActionPerformed]", "Texto Digitado : " + textOld);
...
" Kepler Nicolai Alves Freitas  - 31222198"
    ...
    private void MenuOpcoesActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MenuOpcoesActionPerformed
        if(frameOpecoes == null){
            ...
            frameOpecoes.FieldArqLog.setText(autolog.getAbsolutePath());
        ...
        autolog = new File(frameOpecoes.FieldArqLog.getText());
    ...
    private void btnConnectarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnConnectarActionPerformed
            ...
            if (aux) {
                ...
                ** Validação nova :                
                ...
                Object tex = JOptionPane.showInputDialog(null, "Voce nao informou qual tipo de criptografia irá utilizar. Informe o tipo de criptografia a ser utilizado"
                        ,"Escolha de Criptografia" , JOptionPane.INFORMATION_MESSAGE,null, tiposCripto, "DES - CBC");
                 System.out.println("Tipo de Criptografia Selecionada : " + tex);
                
                 if (tex == "DES - CBC") {
                     tpCrypt =1; //DES - encryptCBC
                 }else if(tex =="DES - ECB"){
                      tpCrypt = 2; //DES - DESPaddingECB
                 }else if(tex =="AES - CBC"){
                      tpCrypt = 3; //AES - CBC
                 }else if(tex =="AES - ECB"){
                      tpCrypt = 4; //AES - ECB
                 ...
                 //Ponto de Log, apenas para verificar o tipo de criptografia escolhida.
                ...
                if (tpCrypt == 0) {
                    if(JOptionPane.showConfirmDialog(null, "Voce nao informou qual tipo de criptografia irá utilizar. Deseja utilizar DES?\n"
                        ...
                        tpCrypt = 2; //DESPaddingECB
                        //tpCrypt =3; //encryptCBC
                        this.message.setTpCrypt(tpCrypt);
                    }else{
                        tpCrypt = 1;
                        this.message.setTpCrypt(tpCrypt);
                ...
                */
                ...
                switch (this.message.getTpCrypt()) {
                    case 1:
                        validaCrypt = "[DES-CBC]Ativada";
                        break;
                    case 2:
                        validaCrypt = "[DES-ECB]Ativada";
                        break;
                    case 3:
                        validaCrypt = "[AES - ECB]Ativada";
                        break;
                    case 4:
                        validaCrypt = "[AES - ECB]Ativada";
                        break;
                    case 5:
                        validaCrypt = "[RSA]Ativada";
                        break;
                    default:
                        validaCrypt = "Desativada";
                }
    ...
    public void AutoLog(){
        ...
        try {
            autolog = new File(autolog.getAbsoluteFile() + "\\[Cliente]" + DataArqLog + "_autoLog_" + nome + ".logSc");
            sys.CriaLog(autolog);
    ...
    private void ValidaCriptoEnviaMsg(String textoDigitado) {
        if (this.message.getTpCrypt() == 0) {

            //Sem Criptografia                
            this.message.setText(textoDigitado);
            sys.EscreveLogConsole("ClienteInterface - Metodo btnEnviar[NoCrypt]", "[" + this.message.getName() + "]" + "textOld : " + textoDigitado);
            ...
            try {
                //DES CBC
                sys.EscreveLogConsole("[btnEnviarActionPerformed]", "[" + this.message.getName() + "]" + "[ChaveDES-CBC] : " + chaveDESCBC);
                sys.EscreveLogConsole("[btnEnviarActionPerformed]", "[" + this.message.getName() + "]" + "[DES-CBC]TextoDescriptografado : " + classeDES.decryptCBC(txtbyte, chaveDESCBC));
            ...
            //DES ECB
            sys.EscreveLogConsole("[btnEnviarActionPerformed]", "[" + this.message.getName() + "]" + "[ChaveDES-ECB] : " + Base64.getEncoder().encodeToString(chaveDES.getEncoded()));
            sys.EscreveLogConsole("[btnEnviarActionPerformed]", "[" + this.message.getName() + "]" + "[DES-ECB]TextoDescriptografado : " + classeDES.CriptoDESPaddingECBDecrypter(txtbyte, chaveDES));
        ...
        } else if (this.message.getTpCrypt() == 5) {// Validação se é criptografia AES

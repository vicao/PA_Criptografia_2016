...
public class ClientInterface extends javax.swing.JFrame {
    ...
    private void btnConnectarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnConnectarActionPerformed
        ...
        if (aux) {
            ...
            validaCrypt = (this.message.getTpCrypt() == 6) ? "[AES]Ativada" : "[AES]Desativada";
    ...
    private void ValidaCriptoEnviaMsg(String textoDigitado) {
        ...
        if (this.message.getTpCrypt() == 0) {
            ...
            try {
                ...
                txtbyte = criptografiaAES(textoDigitado);
                this.message.setText("Mensagem Criptografada");
                sys.EscreveLogConsole("[ClienteInterface - Metodo btnEnviarActionPerformed][AES - 4]", "TextoDescriptografado : " + aes.decrypt(txtbyte, chaveAES));
            } catch (Exception ex) {
                Logger.getLogger(ClientInterface.class.getName()).log(Level.SEVERE, null, ex);
            }
            ...
            try {
                ...
                txtbyte = criptografiaAES(textoDigitado);
                this.message.setText("Mensagem Criptografada");
                sys.EscreveLogConsole("[ClienteInterface - Metodo btnEnviarActionPerformed][AES - 5]", "TextoDescriptografado : " + aes.decrypt(txtbyte, chaveAES));
            } catch (Exception ex) {
                Logger.getLogger(ClientInterface.class.getName()).log(Level.SEVERE, null, ex);
            }
